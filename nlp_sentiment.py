...
def score_all_filings(conn: sqlite3.Connection,
    ...
    for _, row in filings_df.iterrows():
        ...
        result = score_single_filing(raw_text, filing_type, scorer, max_tokens)
...
def compute_sector_signals(conn: sqlite3.Connection,
    ...
    for etf, holdings in sector_holdings.items():
        ...
        # Aggregate: mean score, mean confidence
        sector_score = float(latest_per_ticker["raw_score"].mean())
        sector_conf = float(latest_per_ticker["confidence"].mean())
...
def generate_nlp_report(scores_df: pd.DataFrame,
    ...
    if signals_df.empty:
        ...
        vix_str = f"{vix_pct:.1f}%" if vix_pct and not math.isnan(vix_pct) else "N/A"
