...
try:
    ...
    try:
        from sector_rotation.etf_selector import (
            get_selected_tickers as _get_etf_selections,
            get_ticker_asset_class_map as _get_selector_asset_class_map,
        )
        _ETF_SELECTOR_AVAILABLE = True
...
# Regime probability blending for smooth allocation transitions
try:
    from regime_probability_enhancement import compute_blended_allocation_bands
    _REGIME_BLEND_AVAILABLE = True
except ImportError:
    try:
        from sector_rotation.regime_probability_enhancement import compute_blended_allocation_bands
        _REGIME_BLEND_AVAILABLE = True
    except ImportError:
        _REGIME_BLEND_AVAILABLE = False
...
def compute_composite_factor_scores(
    ...
    try:
        ...
        from sector_rotation.regime_detector import load_sector_prices
    ...
    decay = cfg["factor_model"]["mclean_pontiff_decay"]  # 0.74 (flat default)
    # Per-factor McLean-Pontiff decay rates from post-publication attenuation
    # literature (McLean & Pontiff, 2016; Jacobs & Muller, 2020).
    # Factors with higher turnover / crowding attenuate faster.
    per_factor_decay = {
        "alpha": decay,          # 0.74 — baseline
        "HML":   0.69,           # value: moderate crowding
        "RMW":   0.82,           # quality: less crowded, slower decay
        "CMA":   0.76,           # investment: moderate
        "SMB":   0.74,           # size: baseline
    }
    mom_decay = 0.58             # momentum: highest turnover, fastest decay
    ...
    for ticker in sector_wide.columns:
        ...
        adjusted_alpha = raw_alpha * per_factor_decay["alpha"]

        # Apply per-factor decay to loadings
        adj_hml = loadings.get("HML", 0) * per_factor_decay["HML"]
        adj_rmw = loadings.get("RMW", 0) * per_factor_decay["RMW"]
        adj_cma = loadings.get("CMA", 0) * per_factor_decay["CMA"]
        adj_smb = loadings.get("SMB", 0) * per_factor_decay["SMB"]
        ...
        composite = (
            ...
            0.15 * _normalize_signal(adj_hml, scale=0.5) +
            0.20 * _normalize_signal(adj_rmw, scale=0.5) +
            0.10 * _normalize_signal(adj_cma, scale=0.5) +
            0.05 * _normalize_signal(adj_smb, scale=0.5) +
            0.20 * (mom * mom_decay)
        )
...
def run_cvar_optimization(
    ...
    bounds = _compute_allocation_bounds(returns.columns.tolist(), regime, cfg, factor_scores, regime_probs=regime_probs)
...
def _compute_allocation_bounds(
    ...
    # Pre-compute blended bands if regime probabilities are available
    blended_bands = None
    if regime_probs is not None and _REGIME_BLEND_AVAILABLE:
        try:
            blended_bands = compute_blended_allocation_bands(regime_probs, cfg)
            logger.info("Using probability-blended allocation bands (p_panic=%.2f, p_defense=%.2f, p_offense=%.2f)",
                        regime_probs.get("panic", 0), regime_probs.get("defense", 0), regime_probs.get("offense", 0))
        except Exception as e:
            logger.warning("Blended bands failed (%s), falling back to discrete", e)
    ...
    for ticker in tickers:
        ...
        # Use blended bands when available, otherwise discrete lookup
        if blended_bands is not None and ac in blended_bands:
            group_lo, group_hi = blended_bands[ac]
        else:
...
def run_portfolio_optimization(
    ...
    if regime is None:
        ...
        regime_probs = state.get("regime_probabilities")  # dict or None
    else:
        regime_probs = None
